...
function Practice() {
  ...
  const solveHectoc = (seq) => {
    if (!seq || seq.length !== 6 || !/^[1-9]{6}$/.test(seq)) {
      return "Invalid sequence";
    }

    const digits = seq.split('').map(Number);

    // Helper function to evaluate expression using mathjs
    const evaluate = (expr) => {
      try {
        const result = math.evaluate(expr);
        return Math.abs(result - 100) < 0.0001; // Floating-point precision
      } catch (e) {
        return false; // Division by zero, syntax errors, etc.
      }
    };

    // Recursive solver to try all operators and parentheses
    const solve = (currentExpr, remainingDigits) => {
      // Base case: all digits used, check if it equals 100
      if (remainingDigits.length === 0) {
        if (evaluate(currentExpr)) {
          return currentExpr;
        }
        return null;
      }
      ...
      const rest = remainingDigits.slice(1);

      // If this is the first digit, start with it
      if (currentExpr === "") {
        return solve(digit.toString(), rest);
      }

      // Try all operators
      const operators = ['+', '-', '*', '/', '^'];
      for (const op of operators) {
        const newExpr = `${currentExpr}${op}${digit}`;
        if (rest.length === 0 && evaluate(newExpr)) {
          return newExpr; // Final expression works
        }
        const result = solve(newExpr, rest);
        if (result) return result;
      }

      // Try grouping with parentheses (e.g., (currentExpr + digit) * next)
      if (rest.length > 0) {
        for (const op of operators) {
          const parenExpr = `(${currentExpr}${op}${digit})`;
          const parenResult = solve(parenExpr, rest);
          if (parenResult) return parenResult;

          // Try nested parentheses with next digit
          if (rest.length > 1) {
            const nextDigit = rest[0];
            const nextRest = rest.slice(1);
            for (const op2 of operators) {
              const nestedExpr = `(${currentExpr}${op}${digit}${op2}${nextDigit})`;
              const nestedResult = solve(nestedExpr, nextRest);
    ...
    // Start solving with empty expression and all digits
    const solution = solve("", digits);
    return solution || "No exact solution found";
  };
  ...
  const checkAnswer = () => {
    ...
    try {
      const result = math.evaluate(userAnswer);
        ...
        <div className="flex flex-col md:flex-row justify-center items-start gap-6 p-4 w-full max-w-6xl">
            ...
            {gameStatus === 'playing' && (
              <>
                <div className="mb-6">
                  ...
                  <p className={styles.instruction}>Insert +, -, *, /, ^ and parentheses between the digits</p>
                </div>
                ...
                <div className="flex justify-between mb-4 gap-4">
                  <button onClick={checkAnswer} className={styles.primaryButton}>
                    Check Answer
                  </button>
          ...
          <div className="w-full md:w-1/3 bg-[#081c30] rounded-lg shadow-lg p-6 text-white relative z-1 border border-[rgba(255,255,255,0.05)] mt-6 md:mt-0">
            ...
            <div className={styles.example}>
              ...
              <p className="text-sm italic mt-3 text-[rgba(255,255,255,0.5)]">1 + (9) × (11) = 100</p>
            </div>
            ...
            <div className="mt-6">
              ...
              <p className="text-sm text-[rgba(255,255,255,0.7)]">You must keep the digits in the original order!</p>
            </div>
          </div>
        </div>
      </div>
